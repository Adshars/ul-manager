...
class HiveSource @Inject constructor(private val api: HiveApi) {
    ...
    suspend fun getByQrCode(code: String): Result<Hive> = safeCall {
        val response = api.getByQrCode(code)
        val body = response.bodyOrThrow()
        response.cacheEtag(body.id)
        body.toDomain()
    }

    suspend fun create(request: CreateHiveRequest): Result<Hive> = safeCall {
    ...
    suspend fun update(id: Long, request: UpdateHiveRequest): Result<Hive> = safeCall {
    ...
    suspend fun regenerateQr(id: Long): Result<Hive> = safeCall {
        val response = api.regenerateQr(id, etags.require(id))
        val body = response.bodyOrThrow()
        response.cacheEtag(id)
        body.toDomain()
    }
