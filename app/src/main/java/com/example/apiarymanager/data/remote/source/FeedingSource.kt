...
@Singleton
class FeedingSource @Inject constructor(private val api: FeedingApi) {

    private val etags = ConcurrentHashMap<Long, String>()

    suspend fun getByHive(hiveId: Long, page: Int = 1, pageSize: Int = 50): Result<List<Feeding>> = safeCall {
        api.getByHive(hiveId, page, pageSize).items.map { it.toDomain() }
    }

    suspend fun getById(id: Long): Result<Feeding> = safeCall {
        val response = api.getById(id)
        response.cacheEtag(id)
        response.bodyOrThrow().toDomain()
    }

    suspend fun create(request: CreateFeedingRequest): Result<Feeding> = safeCall {
    ...
    suspend fun update(id: Long, request: UpdateFeedingRequest): Result<Feeding> = safeCall {
