...
@Singleton
class ApiarySource @Inject constructor(private val api: ApiaryApi) {

    private val etags = ConcurrentHashMap<Long, String>()

    suspend fun getAll(page: Int = 1, pageSize: Int = 50): Result<List<Apiary>> = safeCall {
        api.getAll(page, pageSize).items.map { it.toDomain() }
    }

    suspend fun getById(id: Long): Result<Apiary> = safeCall {
        val response = api.getById(id)
        response.cacheEtag(id)
        response.bodyOrThrow().toDomain()
    }

    suspend fun create(request: CreateApiaryRequest): Result<Apiary> = safeCall {
    ...
    suspend fun update(id: Long, request: UpdateApiaryRequest): Result<Apiary> = safeCall {
