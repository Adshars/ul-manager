...
fun HarvestFormScreen(
    ...
    harvestId: Long?,
    onNavigateBack: () -> Unit,
    viewModel: HarvestFormViewModel = hiltViewModel()
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { when (it) {
            HarvestFormEvent.NavigateBack        -> onNavigateBack()
            is HarvestFormEvent.ShowMessage      -> snackbarHostState.showSnackbar(it.message)
        } }
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(if (harvestId == null) "Nowe miodobranie" else "Edytuj miodobranie") },
        ...
        Column(
            ...
            ElevatedCard(modifier = Modifier.fillMaxWidth()) {
                Row(modifier = Modifier.fillMaxWidth().clickable { showDatePicker = true }.padding(16.dp), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Column(Modifier.weight(1f)) {
                        Text("Data miodobrania", style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
            ...
            OutlinedTextField(value = uiState.honeyType, onValueChange = viewModel::onHoneyTypeChange, label = { Text("Rodzaj miodu") }, singleLine = true, modifier = Modifier.fillMaxWidth())
            Spacer(Modifier.height(8.dp))
            OutlinedTextField(
                value = uiState.weightKg, onValueChange = viewModel::onWeightKgChange,
                label = { Text("Waga (kg)") }, isError = uiState.weightError != null,
                supportingText = uiState.weightError?.let { { Text(it) } },
                keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Decimal),
                singleLine = true, modifier = Modifier.fillMaxWidth()
            )
            Spacer(Modifier.height(8.dp))
            OutlinedTextField(value = uiState.notes, onValueChange = viewModel::onNotesChange, label = { Text("Notatka") }, minLines = 3, modifier = Modifier.fillMaxWidth())
